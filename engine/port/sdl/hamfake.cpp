...
// POWDER Web (browser/WebAssembly port): emscripten_sleep() requires the
// engine to be linked with -sASYNCIFY.  It lets the otherwise-blocking
...
#ifdef __EMSCRIPTEN__
    // The browser is single-threaded and we cannot block it.  Yield ~one
    // frame (Asyncify unwinds the stack and hands control back to the
    // browser) so queued keyboard/mouse input is delivered, then drive the
    // VBL tick ourselves.  We deliberately do NOT register an SDL timer in
    // the Emscripten build (see ham_StartIntHandler) -- this is its
    // replacement, mirroring glb_VBLTimerCallback().
    emscripten_sleep(16);
    glb_isnewframe = 1;
    if (glb_vblcallback)
	(*glb_vblcallback)();
...
ham_StartIntHandler(u8 intno, void (*fp)())
{
    ...
    glb_vblcallback = fp;
#if !defined(USE_VIRTUAL_SCREEN) && !defined(__EMSCRIPTEN__)
    SDL_AddTimer(16, glb_VBLTimerCallback, 0);
#endif
    // Under Emscripten the VBL tick is driven from hamfake_awaitEvent()
    // instead of an SDL timer (SDL1 timers are unreliable in the browser).
}
...
hamfake_setTileSize(int tilewidth, int tileheight)
{
	...
	memset(glbTheTileSet->tiles[i], 0, TILEWIDTH * TILEHEIGHT);
    }
	...
	glbSpriteList[i].data = new u8[TILEWIDTH * TILEHEIGHT * 4];
	memset(glbSpriteList[i].data, 0, TILEWIDTH * TILEHEIGHT * 4);
    }
#ifdef USE_VIRTUAL_SCREEN
